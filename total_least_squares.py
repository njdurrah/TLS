...
def computeBasis(ns):
    ...
    L7 = 3*ns[:,2]*ns[:,2] - 1
    L8 = ns[:,0]*ns[:,2]
    L9 = ns[:,0]*ns[:,0]-ns[:,1]*ns[:,1]
    B = np.stack([L1,L2,L3,L4,L5,L6,L7,L8,L9],-1)
    return B
...
if __name__ == '__main__':
    ...
    def tlstsqr(X, y, scale):
        m = np.concatenate([X, y/scale],-1)
        w = []
        u,s,v = np.linalg.svd(m)
        v = v.T
        xy = v[:B.shape[1],B.shape[1]:]
        yy = v[B.shape[1]:,B.shape[1]:]
        w = -xy.dot(np.linalg.inv(yy))
        return w*scale
    ...
    coeffT = tlstsqr(Bp, vsp, -1)
    imgT = relightSphere(tennis,coeffT)
    outputT = compositeImages(imgT,target)
    print('Total Coefficients:\n'+str(coeffT))
    plt.figure(2)
    plt.imshow(outputT)
    plt.show()
    imsave('outputT.png',outputT)

    coeffS = tlstsqr(Bp, vsp, 255*1.5)
    imgS = relightSphere(tennis,coeffS)
    outputS = compositeImages(imgS,target)
    print('Scaled Coefficients:\n'+str(coeffS))
    plt.figure(3)
    plt.imshow(outputS)
    plt.show()
    imsave('outputS.png',outputS)
